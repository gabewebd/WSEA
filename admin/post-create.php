...
if (isset($_POST['submit_post'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO posts (title, slug, content, meta_description, featured_image, image_alt_text, status, author_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssssi", $title, $slug, $content, $meta_desc, $image_filename, $alt_text, $status, $author_id);
...
<head>
    ...
    <title>Write Post | Danono's Admin</title>
    <link
    ...
    <div class="admin-page" style="padding: 0;">
        ...
        <div class="admin-header" style="margin: 0; width: 100%;">
            ...
            <div class="admin-header-right">
                <a href="../blogs.php" target="_blank" class="btn btn-secondary">
                    <i class="ph ph-eye"></i> Preview Site
                </a>
            </div>
        </div>
            ...
            <div class="editor-layout">
                ...
                <div class="editor-sidebar">
                    ...
                    <div class="sidebar-section">
                        ...
                        <button type="submit" name="submit_post" id="submit-btn" class="btn btn-primary btn-full btn-lg"
                            style="margin-top: 15px;">
                            <i class="ph ph-paper-plane-tilt"></i> <span id="btn-text">Publish Post</span>
                        </button>
                    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            function updateButtonText() {
                ...
                if (status === 'draft') {
                    $('#btn-text').text('Save Draft');
                    $('#submit-btn i').attr('class', 'ph ph-floppy-disk');
                } else {
                    $('#btn-text').text('Publish Post');
                    $('#submit-btn i').attr('class', 'ph ph-paper-plane-tilt');
                }
            }
